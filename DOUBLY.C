...
void insertAfter(Node *start,int value,int after)
{
	...
	else start=insertEnd(start,value);
}
Node* insertBefore(Node *start,int value,int before)
{
	Node *ptr,*p;
	if(start->data==before)
	{
		return(insertBeg(start,value));

	}
	ptr=(Node *)malloc(sizeof(Node));
	ptr->data=value;
	p=start;
	while(p->data!=before && ptr!=NULL)
	{
		p=p->next;
	}
	ptr->next=p;
...
void main()
{
	...
	do
	{
		...
		switch(choice)
		{
			...
			scanf("%d",&value1);
			start=insertBefore(start,value,value1);
			break;
			...
			break;
			case 12:if(start==NULL) printf("Empty list\n");
			else display(start);
			break;
			case 13:if(start==NULL) printf("Empty list\n");
			else reverse(start);
			break;
			case 14:break;
